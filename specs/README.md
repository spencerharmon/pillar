...
| `BreakGlassRecovery.tla` | `RecoveryNeedsQuorum`, `SubThresholdNeverRecovers`, `RecoveryRotatesAndRevokes`, `RecoveryHonorsContainment`, `RecoveryHonorsEpoch`, `FreshMarkBounded`, `TypeOK` | break-glass recovery via admin quorum (user-management roadmap A4, spec half): an M-of-N quorum of currently-fresh admins co-signs a one-time recovery credential for a subject that lost its key; the recovery rotates the key generation (old key dead forever) and strictly advances the global revocation epoch, regrants only a subset of the subject's prior surviving authority (never an escalation), and the freshly-minted credential is CONTAINED (no usable authority) until onboarding completes — gates `um-mofn-breakglass-recovery-impl` | `docs/user-management-roadmap.md` A4 |
