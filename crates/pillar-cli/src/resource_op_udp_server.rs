...
/// no `PILLAR_RESOURCE_OP_UDP_BIND` override is given — chosen distinct from
/// every other port this binary uses: web `8642` ([`crate::run::DEFAULT_WEB_PORT`]),
/// the TCP health probe `8643` ([`crate::health::DEFAULT_HEALTH_PORT`]), and the
...
mod tests {
    ...
    /// A `Session` control op dispatches over the wire and is member-gated
    /// fail-closed: an unadmitted signer's `session ls` is refused.
    #[test]
    fn a_session_op_from_an_unadmitted_signer_is_refused_fail_closed() {
        let keys = ResourceOpServerKeys::derive(test_keys().0, test_keys().1);
        let (cell, group) = test_keys();
        let (signer, secret) =
            principal_from_seed(&Seed::from_bytes(b"client-d".to_vec())).expect("principal");
        let op = pillar_ops::ControlOp::Session(pillar_ops::SessionOp::List {
    ...
    /// A `Wot` control op dispatches over the wire and is member-gated
    /// fail-closed: an unadmitted signer's `wot graph` view is refused.
    #[test]
    fn a_wot_op_from_an_unadmitted_signer_is_refused_fail_closed() {
        let keys = ResourceOpServerKeys::derive(test_keys().0, test_keys().1);
        let (cell, group) = test_keys();
        let (signer, secret) =
            principal_from_seed(&Seed::from_bytes(b"client-e".to_vec())).expect("principal");
        let op = pillar_ops::ControlOp::Wot(pillar_ops::WotOp::Graph);
        let msg = seal_control_op(&op, &cell, &group, signer.signing, &secret.signing);
        let reply = handle_datagram(
            &msg.to_canonical_cbor().expect("encode"),
            &Arc::new(Mutex::new(WebAuthContext::new(
    ...
    /// A `User` lifecycle act dispatches over the wire and is refused
    /// fail-closed for an unadmitted signer (`user disable` from a stranger).
    #[test]
    fn a_user_op_from_an_unadmitted_signer_is_refused_fail_closed() {
        let keys = ResourceOpServerKeys::derive(test_keys().0, test_keys().1);
        let (cell, group) = test_keys();
        let (signer, secret) =
            principal_from_seed(&Seed::from_bytes(b"client-f".to_vec())).expect("principal");
        let op = pillar_ops::ControlOp::User(pillar_ops::UserOp::Disable {
            handle: "victim".into(),
        });
        let msg = seal_control_op(&op, &cell, &group, signer.signing, &secret.signing);
        let reply = handle_datagram(
    ...
    #[test]
    fn a_trust_op_from_an_unadmitted_signer_is_refused_fail_closed() {
        let keys = ResourceOpServerKeys::derive(test_keys().0, test_keys().1);
        let (cell, group) = test_keys();
        let (signer, secret) =
            principal_from_seed(&Seed::from_bytes(b"client-h".to_vec())).expect("principal");
        let op = pillar_ops::ControlOp::Trust(pillar_ops::TrustOp::GrantAdd {
    ...
    #[test]
    fn an_iam_op_from_an_unadmitted_signer_is_refused_fail_closed() {
        let keys = ResourceOpServerKeys::derive(test_keys().0, test_keys().1);
        let (cell, group) = test_keys();
        let (signer, secret) =
            principal_from_seed(&Seed::from_bytes(b"client-i".to_vec())).expect("principal");
        let op = pillar_ops::ControlOp::Iam(pillar_ops::IamOp::RoleAdd {
            name: "deployer".into(),
            capabilities: vec!["resource:apply".into()],
        });
        let msg = seal_control_op(&op, &cell, &group, signer.signing, &secret.signing);
        let reply = handle_datagram(
            &msg.to_canonical_cbor().expect("encode"),
