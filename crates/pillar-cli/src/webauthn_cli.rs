...
/// `pillar webauthn name --credential-id <b64url> --label <L>`: relabel one
/// of the caller's credentials over HTTP (no hardware needed). Purely
/// cosmetic — unlike `revoke`, naming the caller's last credential is always
/// allowed (it never disables 2FA), so there is no confirmation gate here.
fn name(args: &[String]) -> Result<String, String> {
    ...
    let label = parsed
        .get("label")
        .ok_or("webauthn name requires --label <text>")?;
    let (authority, _host) = domain_from(&parsed)?;
    let token = token_from(&parsed)?;
