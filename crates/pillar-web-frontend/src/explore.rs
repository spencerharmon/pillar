...
async fn fetch_text(path: &str, token: Option<&str>) -> Result<FetchOutcome, JsValue> {
    ...
    // The session token travels as a `token=` query param, matching every
    // other `/portal/obs/*` endpoint's convention (see web_serve.rs
    // `dispatch_obs_*`, which read it via `query_value(path, "token")`). The
    // node's session tokens are URL-safe, so no extra encoding is applied
    // (identical to `portal::get_url`).
    let full = match token {
        Some(t) if !t.is_empty() => {
            let sep = if path.contains('?') { '&' } else { '?' };
            format!("{path}{sep}token={t}")
        }
        _ => path.to_string(),
    };
    ...
    let resp_value =
        wasm_bindgen_futures::JsFuture::from(window.fetch_with_str_and_init(&full, &opts)).await?;
...
#[cfg(feature = "yew")]
/// The guided PSL query builder for the `traces` signal kind — the exact
/// analogue of [`ExploreProfilesBuilder`] for [`TRACE_KIND`], composing its
/// query via [`build_trace_query`] and pivoting through
/// [`correlate_candidates_traces`]. Submits the composed [`PslQuery::to_text`]
/// to [`ExploreBuilderProps::query_path`] and renders the real matched spans.
#[function_component(ExploreTracesBuilder)]
pub fn explore_traces_builder(props: &ExploreBuilderProps) -> Html {
    let auth = use_auth();
    let label_keys: UseStateHandle<Vec<String>> = use_state(Vec::new);
    let where_key_input = use_state(String::new);
    let where_value_input = use_state(String::new);
    ...
    let on_run = {
        ...
        Callback::from(move |_| {
            let predicates: Vec<(String, String)> = (*where_rows)
                ...
                .collect();
            let correlate = (*correlate_anchor).map(|anchor| (60u64, anchor));
            let Ok(query) = build_trace_query(&[], &predicates, *range_seconds, correlate) else {
                return;
            };
            let text = query.to_text();
            let path = query_path;
    ...
    html! {
        <section data-panel="explore-traces">
            <h2>{ "Explore \u{2014} traces" }</h2>
            <div data-role="predicate-builder">
                <input data-role="where-key" list="explore-traces-label-keys"
                    value={(*where_key_input).clone()} oninput={on_key_input} />
                <datalist id="explore-traces-label-keys">
                    { for label_keys.iter().map(|k| html! { <option value={k.clone()} /> }) }
                </datalist>
                <input data-role="where-value" list="explore-traces-label-values"
                    value={(*where_value_input).clone()} oninput={on_value_input} />
                <datalist id="explore-traces-label-values">
                    { for label_values.iter().map(|v| html! { <option value={v.clone()} /> }) }
                </datalist>
                <button data-role="add-predicate" onclick={on_add_predicate}>{ "Add" }</button>
            </div>
            ...
            <section data-panel="correlate">
                <h3>{ "Correlate" }</h3>
                { for correlate_candidates_traces().into_iter().map(|kind| {
                    let correlate_anchor = correlate_anchor.clone();
                    let label = format!("{kind:?}");
                    let onclick = Callback::from(move |_| correlate_anchor.set(Some(kind)));
                    html! { <button data-role="correlate-kind" onclick={onclick}>{ label }</button> }
...
#[cfg(feature = "yew")]
/// The guided PSL query builder for the `metadata` signal kind — the exact
/// analogue of [`ExploreProfilesBuilder`] for [`METADATA_KIND`], composing its
/// query via [`build_metadata_query`] and pivoting through
/// [`correlate_candidates_metadata`]. Renders the real matched label-set
/// (metadata) signals returned by [`ExploreBuilderProps::query_path`].
#[function_component(ExploreMetadataBuilder)]
pub fn explore_metadata_builder(props: &ExploreBuilderProps) -> Html {
    let auth = use_auth();
    let label_keys: UseStateHandle<Vec<String>> = use_state(Vec::new);
    let where_key_input = use_state(String::new);
    let where_value_input = use_state(String::new);
    ...
    html! {
        <section data-panel="explore-metadata">
            <h2>{ "Explore \u{2014} metadata" }</h2>
            <div data-role="predicate-builder">
                <input data-role="where-key" list="explore-metadata-label-keys"
                    value={(*where_key_input).clone()} oninput={on_key_input} />
                <datalist id="explore-metadata-label-keys">
                    { for label_keys.iter().map(|k| html! { <option value={k.clone()} /> }) }
                </datalist>
                <input data-role="where-value" list="explore-metadata-label-values"
                    value={(*where_value_input).clone()} oninput={on_value_input} />
                <datalist id="explore-metadata-label-values">
                    { for label_values.iter().map(|v| html! { <option value={v.clone()} /> }) }
                </datalist>
                <button data-role="add-predicate" onclick={on_add_predicate}>{ "Add" }</button>
            </div>
            ...
            <section data-panel="correlate">
                <h3>{ "Correlate" }</h3>
                { for correlate_candidates_metadata().into_iter().map(|kind| {
                    let correlate_anchor = correlate_anchor.clone();
                    let label = format!("{kind:?}");
                    let onclick = Callback::from(move |_| correlate_anchor.set(Some(kind)));
                    html! { <button data-role="correlate-kind" onclick={onclick}>{ label }</button> }
