...
mod yew_impl {
    ...
    pub(crate) fn topology_tile() -> Html {
        ...
        let fd = use_state(Vec::<String>::new);
        let fields = use_state(TopoFields::default);
        let msg = use_state(|| None::<(String, bool)>);
        let busy = use_state(|| false);

        let refresh_tree = {
            let (auth, tree) = (auth.clone(), tree.clone());
            Callback::from(move |_: ()| {
                let Some(token) = auth.token.clone() else {
                    return;
                };
                ...
                spawn_local(async move {
                    if let Ok(r) =
                        http("GET", &get_url("/portal/topology/tree", &token, &[]), None).await
                    {
                        if r.ok() {
                            tree.set(r.body);
                        } else {
        ...
        let show_mismatches = {
            ...
            Callback::from(move |_: MouseEvent| {
                ...
                spawn_local(async move {
                    if let Some(lines) =
                        get_lines(auth, get_url("/portal/topology/mismatches", &token, &[])).await
                    {
                        mismatches.set(if lines.is_empty() {
                            vec!["No mismatches.".to_owned()]
                        } else {
                            lines
                        });
                    }
        ...
        let declare = {
            let (auth, busy, msg, fields, refresh_tree) = (
                auth.clone(),
                busy.clone(),
                msg.clone(),
                fields.clone(),
            ...
            Callback::from(move |_: MouseEvent| {
                ...
                let (busy, msg, refresh_tree) = (busy.clone(), msg.clone(), refresh_tree.clone());
                busy.set(true);
                spawn_local(async move {
        ...
        let compute_spread = {
            let (auth, busy, fd, fields) = (auth.clone(), busy.clone(), fd.clone(), fields.clone());
            Callback::from(move |_: MouseEvent| {
                if *busy {
                    return;
                }
                ...
                let f = (*fields).clone();
                let tier = if f.fd_tier.trim().is_empty() {
                    "rack"
                } else {
                    f.fd_tier.trim()
                };
                let url = get_url(
                    ...
                    &token,
                    &[("tier", tier), ("nodes", f.fd_nodes.trim())],
                );
                let (auth, busy, fd) = (auth.clone(), busy.clone(), fd.clone());
                busy.set(true);
                spawn_local(async move {
        ...
        let tf = fields.clone();
        let topo_field = move |set: fn(&mut TopoFields, String)| {
            let tf = tf.clone();
            Callback::from(move |e: InputEvent| {
        ...
        html! {
            <div class="tile" id="topology-tile">
                <h3>{ "Topology explorer" }</h3>
                <p>{ "The tier tree with health/capacity, the label editor, and the failure-domain overlay." }</p>
                <PendingButton id="topology-tree-refresh-btn" label="Refresh tree" busy={*busy}
                    onclick={Callback::from(move |_| refresh_tree.emit(()))} />
                <button type="button" id="topology-mismatches-btn" onclick={show_mismatches}>{ "Show mismatches" }</button>
                <pre id="topology-tree">{ (*tree).clone() }</pre>
                <div id="topology-mismatches">
                ...
                { message_line("topology-label-msg", &msg) }
            </div>
        }
    }
