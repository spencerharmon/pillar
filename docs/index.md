...
| [database-management.md](database-management.md) | User-facing guide to managing your cell's data with the `pillar` CLI: catalog discovery, key/value + document + SQL + PSL query surfaces, collections and access control as resources, and viewing/editing the default ResourceSet (system collections, default rolebindings, `CollectionPolicy`, retention) |
| [data-inspection.md](data-inspection.md) | Inspecting the layers beneath the folded views: the op log (`pillar log`), the storage layout (document snapshot+tail vs time-series retention blocks), and each content-addressed IPFS object by CID (`pillar object`), plus the portal Collection Explorer drill-down |
