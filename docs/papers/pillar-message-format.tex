...
multihash of its canonical CBOR encoding. A body's seal is chosen by \emph{how many
parties must unseal it}: the transport-frame seal (a datagram fans out to many ingest
nodes) and the \code{PillarMessage} content seal (a message reaches every cell node)
are both \emph{convergent}---the key is portable and the nonce is derived (HKDF) from
the cell key and the content address of the plaintext, so identical bodies produce an
identical \Cid{} on every node and redundant sprayed datagrams and duplicate blocks
dedupe, while distinct plaintexts never share a nonce (AEAD-safe). The \emph{contents}
of a direct-message Op reach one recipient, so they are instead \emph{random}-sealed
(unique per message, \emph{not} content-deduped) and referenced by \Cid{} from a
cell-sealed wrapper; some control bodies ride unencrypted. streamdb
...
\code{DedupUnderEncryption}, \code{CellConfidential},
\code{NoNonceReuseAcrossDistinctPlaintext} (envelope), and
\code{Convergent\-CellDedup}, \code{RcptRandomDistinctByEntropy},
\code{RcptOpaqueToNonHolder} (body treatments) must be green under TLC before any Rust.
\end{abstract}
...
\section{Three seal treatments}
A body's seal is decided by \emph{how many parties must unseal it}
(Table~\ref{tab:seals}). The \textbf{transport frame seal} (a datagram fanned out to
many ingest nodes) and the \textbf{content seal} on \code{PillarMessage.body} (a
message reaching every cell node) each require \emph{many} recipients to unseal, so
both are \emph{convergent}: a portable key and a deterministic nonce make identical
content byte-identical, which is what lets redundant sprayed datagrams and duplicate
blocks dedupe. The \textbf{application-content seal} on the \emph{contents} of a
direct-message Op reaches \emph{one} recipient, so it may draw a \emph{random}
per-message nonce---the message is unique and canonical for itself, so same-text
messages at different times stay distinct and are not content-deduped. On QUIC/TCP
the transport's native TLS plays the frame-seal role; a control body may ride
unencrypted.

\begin{table}[t]
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.12\columnwidth}p{0.235\columnwidth}p{0.225\columnwidth}p{0.24\columnwidth}@{}}
\toprule
 & \textbf{transport frame} & \textbf{content seal} & \textbf{app-content} \\
\midrule
layer     & pillar-UDP datagram & \code{PillarMessage} body & Op contents \\
unseal    & many (ingest nodes) & many (cell nodes) & \textbf{one} (recipient) \\
key       & session key $K_s$ & cell group key & recipient sealed-box \\
nonce     & \textbf{convergent} & \textbf{convergent} & \textbf{random} \\
dedup     & redundant datagrams & identical blocks & none (unique) \\
\bottomrule
\end{tabular}
\caption{The seal is chosen by recipient count. The two multi-node layers are
convergent (so redundant/duplicate copies dedupe); the single-recipient
application-content seal is random (so each direct message is a distinct record—
exactly-once is an application-layer concern, not the wire format). A control body
may be unencrypted. The content seal is transport-agnostic and travels with the record;
the transport frame seal is
...
\hspace*{1.2em}body\_sealed : Ciphertext\ // per treatment (\S5)\\
\}\\[3pt]
enum Body \{\ // what body\_sealed decodes to\\
\hspace*{1.2em}StreamOp(StreamOpBody),\ // op-log; also typed\\
\hspace*{3.0em}// UserMessage/NodeMessage/KeyOffer:\\
\hspace*{3.0em}// \{recipient, sealed\_cid\} (\S5b)\\
\hspace*{1.2em}Signal(SignalBody),\ \ \ // one of 5 kinds\\
\hspace*{1.2em}Control(ControlBody),\ \ // may be plaintext (\S5c)\\
\}
\end{quote}
...
\section{Seal treatments}\label{sec:seal}
\textbf{(a) Convergent, where many must unseal.} A \code{PillarMessage} reaches every
node in its cell and they must \emph{all} decrypt, so the content seal is
\emph{convergent}: \emph{the recipient is the cell; any cell node decrypts; the \Cid{}
is the same across nodes; we deliver BOTH dedup AND encryption.} Na\"ively this is
impossible---\code{pillar-crypto::seal\_symmetric}
...
so equality never leaks across classes. This convergent seal is what a StreamOp, an
obs signal, and a direct message's cell \emph{wrapper} get; it is exactly what a
random nonce would break, and why the two multi-node layers (content seal and
transport frame) are convergent (proven in \code{PillarMessageFormat.tla}).

\textbf{(b) Random, where one recipient unseals.} The \emph{contents} of a
direct-message Op---a typed \code{UserMessage}/\code{NodeMessage}/\code{KeyOffer}---are
sealed to \emph{one} recipient (X25519 sealed-box) and referenced by \Cid{} from a
cell-sealed wrapper. The cell sees the op and the reference but never the plaintext
(it coordinates delivery without reading it). Because only one party unseals, the
seal draws a \emph{random} per-message nonce, and should: the message is unique and
canonical for itself, so two same-text messages at different times are \emph{distinct}
records with distinct \Cid{}s---\emph{not} content-deduped. The only dedup here is the
byte-identity of truly redundant copies (one message sprayed to many nodes).
\emph{Exactly-once coordination is an application-layer concern (a CP-variant streamdb
or other primitive), not the wire format.}

\textbf{(c) Unencrypted, for one-shot non-addressed control.} Some bodies---typically
libp2p control messages---are sealed once by the transport and not content-addressed,
so they ride as plaintext inside the envelope; any holder reads them. Treatments (b)
and (c) are proven in \code{PillarBodySeals.tla} (\code{RcptOpaqueToNonHolder},
\code{RcptRandomDistinctByEntropy}, \code{DedupByCid}, \code{PlainReadableWithoutKey}).
...
Table~\ref{tab:inv} lists the envelope/content and per-body-treatment obligations,
all green under TLC in \code{specs/PillarMessageFormat.tla} (convergent envelope $+$
version negotiation) and \code{specs/PillarBodySeals.tla} (the three seal
treatments). The transport-frame obligations (portable
...
\midrule
\multicolumn{2}{@{}l}{\emph{envelope $+$ convergent content seal (\code{PillarMessageFormat.tla})}}\\
...
\midrule
\multicolumn{2}{@{}l}{\emph{per-body seal treatments (\code{PillarBodySeals.tla})}}\\
\code{Convergent\-CellDedup} & a cell-sealed body's \Cid{} is a pure function of (cell, content); identical content collapses, distinct never collides \\
\code{RcptRandom\-DistinctByEntropy} & a random single-recipient seal binds per-message entropy; same-content direct messages at different times stay distinct (not deduped) \\
\code{RcptOpaque\-ToNonHolder} & a recipient-sealed Op body opens only for the recipient's key-holder; the cell holds only the referenced \Cid{} \\
\code{DedupBy\-Cid}, \code{Plain\-Readable\-WithoutKey} & redundant/duplicate copies collapse by \Cid{}; an unencrypted control body is readable with no key \\
\bottomrule
\end{tabular}
\caption{TLA\textsuperscript{+} obligations across the two envelope specs: the
convergent envelope $+$ version negotiation (\code{PillarMessageFormat.tla}) and the
three per-body seal treatments (\code{PillarBodySeals.tla}).}
\label{tab:inv}
\end{table}
