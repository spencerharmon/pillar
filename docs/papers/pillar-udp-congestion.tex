...
\begin{enumerate}\itemsep2pt
  ...
  round is capped by a \emph{redundancy allowance} carried in the header; the
  allowance scales \emph{down} toward a single copy on a clean, cheap path and
  \emph{up} as measured loss rises, so redundancy is the default delivery
  posture---dynamically bounded to what conditions warrant---rather than a mode
  entered only for links already known to be bad.
...
\begin{tikzpicture}[
  ...
  gray/.style={-{Stealth[length=1.8mm]},semithick,pgray},
  ...
  \def\xC{0} \def\xA{3.4} \def\xB{6.8} \def\xE{10.2} \def\xD{13.6}
  \def\ybot{-9.6}
  \foreach \x in {\xC,\xA,\xB,\xE,\xD}
     \draw[life] (\x,-0.45) -- (\x,\ybot);
  \node[part] at (\xC,0) {client};
  \node[part] at (\xA,0) {ingest n1};
  \node[part] at (\xB,0) {ingest n2};
  \node[part] at (\xE,0) {ingest n3};
  \node[part,shape=cylinder,shape border rotate=90,aspect=0.3,fill=pcell] at (\xD,0) {streamDB};
  % client setup
  \node[note,anchor=north,text width=44mm] at (\xC,-0.85)
     {join cell $\to$ receive node list; open\\ redundant client-initiated conns\\ (each opens its own NAT return path)};
  % spray, allowance-bounded; n2 path lossy
  \draw[msg]   (\xC,-2.5) -- node[lbl]{spray (CID, allowance $W$, TTL)} (\xA,-2.5);
  \draw[lossy] (\xC,-3.05) -- node[lblb]{\textcolor{pbad}{$\times$ lossy path}} (\xB,-3.05);
  \draw[msg]   (\xC,-3.6) -- node[lblb]{redundant copy (dispersed IP)} (\xE,-3.6);
  % dedup at DB
  \draw[msg]  (\xA,-4.3) -- node[lbl]{inject (first copy anywhere)} (\xD,-4.3);
  \draw[gray] (\xE,-4.85) -- node[lblb]{later copy: same CID $\Rightarrow$ dedup} (\xD,-4.85);
  \node[note,anchor=north,text width=60mm] at ($(\xA,-5.35)!0.5!(\xE,-5.35)$)
     {\code{ExactlyOnce}: injected $\le 1$ (safety) \quad\textbullet\quad
      \code{SprayWidthBounded}: round $\le$ allowance (safety)};
  % reply from a different node than ingested
  \draw[gray] (\xD,-6.55) -- node[lbl]{best path} (\xE,-6.55);
  \draw[ret]  (\xE,-7.1) -- node[lblb]{reply: \emph{any} well-placed node, WoT-signed (not the ingester)} (\xC,-7.1);
  \node[note,anchor=north,draw=pgood,text width=70mm] at ($(\xC,-7.65)!0.5!(\xD,-7.65)$)
     {adversary may flip \emph{any} links lossy, provided $\ge 1$ dispersed path stays good
      $\Rightarrow$ \code{Delivered} (liveness); coverage accumulates monotonically across retransmit rounds};
\end{tikzpicture}
\caption{\textbf{One-to-many, in time.} The temporal view of
Fig.~\ref{fig:onetomany}. The client dials a dispersed set of ingest nodes
(client-initiated, so each connection opens its own return path through NAT),
then sprays an allowance-bounded set of CID-signed copies. The path to
\code{n2} is lossy and its copy is dropped, but copies reach \code{n1} and
\code{n3}; the first to arrive injects the message and the later one deduplicates
by CID (\code{ExactlyOnce}), while no round exceeds the redundancy allowance
(\code{SprayWidthBounded}). The reply returns from whichever well-placed node has
the best path---not necessarily the one that ingested---and as long as at least
one dispersed path stays good the message is eventually delivered
(\code{Delivered}). Modeled in \code{PillarUdpClient.tla}.}
\label{fig:onetomany-seq}
\end{figure*}
...
\begin{itemize}\itemsep2pt
  ...
  widening diversity past the fixed port-forwarded set. But each punched peer
  consumes a distinct port mapping on the client's NAT: DCUtR removes the
  small-fixed-set bottleneck only to replace it with a \emph{port-exhaustion}
  one, so this tier cannot scale to a large fan-out.
  \item \textbf{IPv4, no DCUtR---floor.} Only the handful of port-forwarded
  ingest nodes the client dialed can answer, over the client-initiated mappings.
  Reply-path redundancy is capped at that small, deployment-fixed set---enough
  for basic resilience but far short of the cell-wide diversity IPv6 unlocks.
\end{itemize}
...
\begin{axis}[
  ...
  xlabel={requested reply-node fan-out $W$},
  ylabel={reachable reply nodes},
  xmin=0,xmax=20, ymin=0, ymax=21,
  xtick={0,4,8,12,16,20}, ytick={0,4,8,12,16,20},
  legend pos=north west, legend style={font=\tiny,fill opacity=0.85,draw=none},
  tick label style={font=\tiny}, label style={font=\tiny},
  grid=both, grid style={gray!18},
]
% IPv6: reachable = requested (unbounded by NAT)
\addplot[pnode,thick,domain=0:20,samples=2] {x};
\addlegendentry{IPv6 (GUA)}
% IPv4 + DCUtR: reachable = min(W, F+B), plateau at port budget (F=2, B=6)
\addplot[pmid,very thick,dashed] coordinates {(0,0)(8,8)(20,8)};
\addlegendentry{IPv4 $+$ DCUtR}
% IPv4 floor: reachable = min(W, F), plateau at port-forwarded set (F=2)
\addplot[pbad,thick,dash dot] coordinates {(0,0)(2,2)(20,2)};
\addlegendentry{IPv4, no DCUtR}
\draw[pmid,densely dotted] (axis cs:8,0) -- (axis cs:8,8);
\node[font=\tiny,pmid,align=left,anchor=west] at (axis cs:8.6,10.4)
   {NAT port-mapping\\ceiling ($F{+}B$)};
\node[font=\tiny,pbad,anchor=west] at (axis cs:8.6,2.0)
   {port-forwarded set ($F$)};
\end{axis}
\end{tikzpicture}
\caption{\textbf{Reply-node reachability by address family.} How many distinct
cell nodes can send \emph{back} to a client that requested a reply fan-out $W$.
With an IPv6 GUA every requested node reaches the client (diagonal, unbounded).
Behind IPv4 NAT only the few port-forwarded ingest nodes the client dialed can
reply (floor, $F$); DCUtR hole-punches additional nodes but saturates at the
client's NAT port-mapping budget ($F{+}B$; here $F{=}2,\,B{=}6$). The plateau is
...
\begin{axis}[
  ...
  ylabel={$\log_{10} P(\text{undelivered})$ on the reply path},
  xmin=0,xmax=0.9, ymin=-20, ymax=0,
  xtick={0,0.1,0.2,0.3,0.5,0.75,0.9}, ytick={0,-5,-10,-15,-20},
  legend pos=south east, legend style={font=\tiny,fill opacity=0.85,draw=none},
  tick label style={font=\tiny}, label style={font=\tiny},
  grid=both, grid style={gray!18},
]
% IPv6: effective reply-path W = 20
\addplot[pnode,thick,mark=*,mark size=1pt] coordinates {
  (0.10,-20)(0.20,-13.98)(0.30,-10.46)(0.50,-6.02)(0.75,-2.50)(0.90,-0.92)};
\addlegendentry{IPv6, $W_{\mathrm{eff}}{=}20$}
% IPv4 + DCUtR: effective W = 8 (port-budget cap)
\addplot[pmid,very thick,mark=triangle*,mark size=1.3pt,dashed] coordinates {
  (0.01,-16.0)(0.02,-13.59)(0.05,-10.41)(0.10,-8.0)(0.20,-5.59)(0.30,-4.18)(0.50,-2.41)(0.75,-1.00)(0.90,-0.37)};
\addlegendentry{IPv4$+$DCUtR, $W_{\mathrm{eff}}{=}8$}
% IPv4 floor: effective W = 2 (port-forwarded set)
\addplot[pbad,thick,mark=square*,mark size=1pt,dash dot] coordinates {
  (0.01,-4.0)(0.05,-2.60)(0.10,-2.0)(0.20,-1.40)(0.30,-1.05)(0.50,-0.60)(0.75,-0.25)(0.90,-0.09)};
\addlegendentry{IPv4 floor, $W_{\mathrm{eff}}{=}2$}
\end{axis}
\end{tikzpicture}
\caption{\textbf{Reliability consequence of the addressing tier.} The reachable
reply-node count (Fig.~\ref{fig:replydiv}) caps the effective reply-path
redundancy $W_{\mathrm{eff}}$, and residual reply loss is $p^{W_{\mathrm{eff}}}$.
IPv6 attains the full redundancy benefit, IPv4$+$DCUtR a bounded fraction of it,
and the IPv4 floor---limited to the port-forwarded set---barely bends the curve.
The request path (client$\to$cell) is unaffected: the client dials whatever
reachable nodes it likes. Lower is better; log scale.}
\label{fig:replyloss}
\end{figure}
...
loss by \emph{relocating} the offered traffic. pillar-UDP is therefore the
\emph{preferred} transport for pillar node-to-node and pillar-native client
traffic---registered as an additional libp2p transport---with its redundancy
allowance scaled down toward a single copy on clean, cheap paths and up only as
measured loss demands, so it pays little where the link is good and covers the
link where it is not. QUIC and TCP remain the fallback and legacy-interoperability
paths---the HTTPS/HTTP-3 ingress for non-pillar clients, and any link where no
pillar-UDP peer is available---not the default: their single-path,
window-throttled posture is precisely what degrades toward zero on the lossy
links pillar targets.
...
role in pillar & fallback / legacy interop & preferred node \& client transport \\
...
sense: it trades bandwidth for reliability, so the redundancy allowance is a
per-connection budget that scales \emph{down} toward a single copy on a clean,
cheap path and \emph{up} only as measured loss warrants. It is this adaptive
bound---not a restriction of the protocol to bad links---that lets pillar-UDP
serve as the default transport without paying a fixed bandwidth multiple; the
high-fan-out regime, not the protocol itself, is what is reserved for degraded
and adversarial links. Bulk transfer is a distinct problem addressed by
